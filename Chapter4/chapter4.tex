...
rather than the evaluation setup. Detection quality and cost-efficiency are
treated as co-equal evaluation axes throughout, not a primary metric with
cost as an afterthought: Chapter~\ref{Chapter3}'s RQ2
(Section~\ref{sec:rq2}) already shows that every reviewed LLM-based
detector trades accuracy against non-trivial per-APK inference cost, so a
candidate's cost profile is reported with the same rigor as its detection
metrics rather than left as a secondary note. As of this writing, one
candidate has a
...
As of this writing, this thesis's candidate set has one member: a
Semantic-Representation-family method built on the LAMD
framework~\cite{Qian2025-ua}, developed and iteratively hardened as
described in Section~\ref{sec:lamd-candidate} below. Additional candidates
representative of the other two families identified in
Chapter~\ref{Chapter3}'s taxonomy (RQ1, Section~\ref{sec:rq1}) --
Retrieval-Augmented Generation and Fine-tuning/Direct Classification --
are planned but not yet implemented, and are not described here since
doing so would mean detailing an architecture and evaluation that does
not yet exist. A prompt-grounding technique layered on top of the
LAMD-based candidate itself was also explored and is reported in
Section~\ref{sec:lamd-candidate}, but it is not a second, independently
evaluated candidate (Chapter~\ref{Chapter6}, Limitations).
...
\textit{[TODO: extend this section with prompts/configuration details for
additional candidates as they are integrated.]}
